...
- **FastAPI application**: `app/main.py` instantiates the API, wires routers for ingestion/data export/webhooks/election metadata, and keeps CORS fully open for now.
- **Database bootstrap**: `app/db_init.py` ensures the target Postgres database exists and then applies `Base.metadata.create_all()`. FastAPI runs `init_db()` on startup, while `scripts/init_db.py` reuses the same helper for manual setup.
...
- **Dev compose endpoint**: `dev.compose.yaml` exposes the API on `http://localhost:8000`; aim your local worker traffic at that base URL during development.
...
All tables are created via `Base.metadata.create_all()` inside `app/db_init.py` (the same helper used by `scripts/init_db.py`). The important pieces for ingestion/worker coordination are:
...
| `parties` | `Text` | JSON string with party → percentage mappings. Worker may send either dict or string and the router will JSON-encode it. |
| `Befragte`, `Zeitraum`, `institute_id`, `forecast_provider`, `scope`, `election_id`, `method_id` | `String` | Optional metadata captured as-is for downstream normalization. |
| `source` | `String` | Required identifier so feeds can be partitioned/deduplicated. |
...
- **Write path requirement**: Never connect to Postgres directly; always POST batches to this endpoint so the FastAPI service applies validation, logging, and insert logic on your behalf.
- **Edge cases**:
...
- `GET /election/results/bundestag` / `GET /election/dates`: Read cached outputs written by the bundestag and election-date scrapers (404s until those JSON files exist).
...
Two supported setups exist so you can mirror the deployed stack or run everything on bare metal while developing a worker. Both rely on the same Postgres defaults (`postgresql+psycopg://postgres:postgres@localhost:5432/pollingapi_dev`).

### Option A – Docker (hot reload, matches production wiring)
1. Copy or create `.env` with any overrides you need (e.g. `DATABASE_URL`, `GITHUB_TOKEN`, `GITHUB_REPO`).
2. `docker compose -f dev.compose.yaml up --build` to start the API and Postgres with code mounted from the repo. The API is served on `http://localhost:8000` with auto reload.
3. Run worker smoke tests against `http://localhost:8000/ingest/polls`.
4. Tear down with `docker compose -f dev.compose.yaml down` when finished. The `postgres-data` and `uv-venv` volumes persist unless you add `-v`.

### Option B – Native Python tooling (fast feedback, no containers)
1. Install Python 3.13 and run `uv sync` to create the virtual environment locally.
2. Start Postgres (e.g. `docker compose up db` using `compose.yaml`, or any local instance listening on port 5432).
3. `uv run python scripts/init_db.py` to create tables and validate connectivity.
4. `uv run fastapi dev app/main.py --host 0.0.0.0 --port 8000` to launch the API with reload.
5. Point your worker or HTTP client at `http://localhost:8000`.

The FastAPI app also invokes `init_db()` during startup, so step 3 doubles as a connectivity check if you want early feedback before hitting the API.

### Database inspection helpers
- Run `psql postgres://postgres:postgres@localhost:5432/pollingapi_dev` to inspect tables after ingestion.
