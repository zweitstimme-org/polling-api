"""Rheinland-Pfalz scraper worker."""

from pollingapi.scraper.config import ScraperConfig
from pollingapi.scraper.wahlrecht import WahlrechtLandScraper


class RpScraper(WahlrechtLandScraper):
    """Scraper for Rheinland-Pfalz state polling data."""

    @classmethod
    def get_config(cls) -> ScraperConfig:
        """Get configuration for Rheinland-Pfalz scraper."""
        return ScraperConfig(
            worker_name="rheinland-pfalz",
            institute_id="various",
            provider="Wahlrecht.de",
            source="html_scraper",
            scope="rheinland-pfalz",
            election_id="Landtagswahl",
            method_id="99",
            urls=[
                {
                    "url": "https://www.wahlrecht.de/umfragen/landtage/rheinland-pfalz.htm",
                    "table_index": 0,
                    "drop_header": 1,
                    "drop_footer": 3,
                },
            ],
            type="wahlrecht_land",
        )


def get_config():
    """Return config for discovery."""
    return RpScraper.get_config()
